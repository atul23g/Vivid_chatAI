...
export const generateLayouts = async (projectId: string, theme: string) => {
  try {
    ...
    const layouts = await generateLayoutsJson(project.outlines); // fixed function name
    console.log("layout generation result:", layouts);
    ...
    return { status: 200, data: layouts.data }; // fixed typo `layout` → `layouts`
  } catch (error) {
    return { status: 500, error: "Internal Server Error" };
  }
};
